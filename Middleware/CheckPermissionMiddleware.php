...
use App\Exceptions\Identity\IdentityValidationException;
use Closure;
...
class CheckPermissionMiddleware
{
    public function handle($request, Closure $next, $permission)
    {
        if (config('accesscontrol.tenant.use_tenant')) {
            ...
            if (!$tenant) {
                throw CustomInvalidException::tenancyLoad()->setStatusCode(Response::HTTP_NOT_FOUND);
            }
        ...
        if (!in_array($permission, $userPermissions)) {
            throw IdentityValidationException::accessDenied()->setStatusCode(Response::HTTP_FORBIDDEN);
        }
